...
export const remove = async (req, res) => {
    ...
    try {
        const book = await Book.findOneAndDelete(condition);
        res.json(book)
    } catch (error) {
        res.status(400).json({
...
export const search = async (req, res) => {
    ...
    try {
        const result = await Book.find( { $text: { $search: SearchString } } ).exec()
        res.json(result)
    } catch (error) {
        res.status(400).json({
...
export const update = async (req, res) => {
    ...
    try {
        const book = await Book.findOneAndUpdate(condition, doc, option);
        res.json(book)
    } catch (error) {
        res.status(400).json({
